...
. as $presets
  ...
  | [$presets.language[] | select(.indent)] | map({ (.name): (.indent |= indentfix | pick(.indent)) }) | add as $pre_indent
  | $usr.language | with_entries(.value."language-servers" += $pre_langs[.key]."language-servers") | . as $user_langs
